...
1. **Source Location**: If the fire origin metadata indicates this IS the source room, label it as "Source Location" in the Executive Summary.
2. **Smoke Migration Principle**: In ANY fire-affected structure, smoke and soot migrate and settle on ALL surfaces in ALL rooms - ceilings, walls, floors, trim, fixtures, contents. Even if a surface appears visually clean, it MUST be classified as at minimum "Light" contamination requiring "Clean" disposition in affected spaces. "No Action" is ONLY appropriate for areas confirmed unaffected by testing/sampling, not by visual observation alone.
3. **Burn Zone Disposition**: In the burn zone (source location), the disposition for ALL materials is "Remove" - including contents, cabinetry, ceiling, walls, lights, and appliances. The ONLY exception is non-porous structural flooring (e.g., tile, concrete) which should be "Clean". In practice, burn zone rooms are gutted.
4. **Room-Level Zone Classification**: For residential rooms and any room ≤1,000 SF, the zone classification applies to the ENTIRE ROOM — do NOT break it into sub-zones or percentages. For commercial/industrial rooms >1,000 SF, sub-zone gradients are permitted if supported by clear physical evidence (e.g., visible damage boundary). The source room (fire origin) = Burn Zone. Adjacent rooms = Near-Field. Rooms separated by multiple rooms/corridors = Far-Field. Use fire origin metadata to determine zone.
5. **Section Consistency**: Zone classifications and severity levels MUST be identical in the Executive Summary, Zone Classification section, and Disposition table.
6. **Complete Output**: Every row in the Disposition table MUST have a specific material/surface name (never generic "Surface" or "Material") and a specific action. Do not output incomplete or placeholder rows.
...
4. Disposition - Clean / Remove / No-action per methodology and Critical Assessment Rules above
